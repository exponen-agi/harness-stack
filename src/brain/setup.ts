...
export const DEFAULT_BRAIN_REPO =
  "https://github.com/exponen-agi/harness-brain.git";
...
export function parseBrainSource(
  raw: string | undefined,
  fallback: BrainSource = "clone",
): BrainSource {
  const v = (raw ?? "").trim().toLowerCase();
  if (["clone", "c", "1", "git"].includes(v)) return "clone";
  if (["scaffold", "s", "2", "local", "copy"].includes(v)) return "scaffold";
  return fallback;
}
...
export async function cloneBrain(
  ...
  if (!(await which("git"))) {
    return {
      ok: false,
      message:
        "git not found on PATH — cannot clone. Re-run with " +
  ...
  if (res.code !== 0) {
    return {
      ok: false,
      message: `git clone failed (exit ${res.code}): ${
 ...
 * always completes.
 */
export async function setupBrain(opts: BrainSetupOptions): Promise<BrainConfig> {
  const { root } = opts;
  ...
  const flagsGiven = Boolean(
    opts.brainPath || opts.brainSource || opts.brainRepo,
  );
  ...
  if (!flagsGiven) {
    if (!interactive) {
      log.detail(
        "harness-brain not configured (pass --brain <path> to enable).",
      );
      return { enabled: false };
    }
    const ans = await confirm(
      "Set up a harness-brain commit-memory for this project?",
      { projectRoot: root, key: "brain", allowAlways: false },
    );
    if (!isApproved(ans)) {
      log.detail(
        "harness-brain declined — add it later with `harness init --brain <path>`.",
      );
      return { enabled: false };
    }
  }
  ...
  if (interactive && !opts.brainSource) {
    const raw = await ask(
      "Source — 1) clone the default harness-brain repo  " +
